---
name: minions-audiences
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Audiences MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-audiences data."
category: ai
subcategory: general
...
# Minions Audiences Agent Skills

Skills for agents operating on the `minions-audiences` toolbox.
...
```bash
# TypeScript
pnpm add @minions-audiences/sdk

# Python
pip install minions-audiences

# CLI
pnpm add -g @minions-audiences/cli
```
...
The `audiences` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-audiences/sdk/schemas';
...
```python
from minions_audiences.schemas import custom_types
...
- This agent only writes to `minions-audiences` — it reads from other toolboxes but never writes to them